...
namespace BugsTrackersScreenCapture
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            using (Mutex mutex = new Mutex(true, "ScreenCapture", out createdNew))
            {
                if (createdNew)
                {
                    ...
                    // fetch settings
                    url = Properties.Settings.Default.ScreenCapture_BugTrackerWebService_BugTrackerWebService;
                    username = ConfigurationManager.AppSettings["username"];//Properties.Settings.Default.username;//
                    password = ConfigurationManager.AppSettings["password"]; //Properties.Settings.Default.password;//
